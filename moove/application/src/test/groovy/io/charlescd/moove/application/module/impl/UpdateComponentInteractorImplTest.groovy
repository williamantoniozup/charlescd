...
class UpdateComponentInteractorImplTest extends Specification {
    ...
    def "should update a existing component"() {
        ...
        def request = new ComponentRequest("new-component-name", 20, 20, 'host', 'gateway', 'namespace')

        def author = getDummyUser()

        def component = new Component(componentId, moduleId, "component-name", LocalDateTime.now(), workspaceId, 10, 10, 'host', 'gateway', 'namespace')
        def updatedComponent = new Component(componentId, moduleId, "new-component-name", LocalDateTime.now(), workspaceId, 20, 20, 'host', 'gateway', 'namespace')
    ...
    def "should throw not found exception when component does not exists"() {
        ...
        def component = new Component("component-id", moduleId, "component-name", LocalDateTime.now(), workspaceId, 10, 10, 'host', 'gateway','namespace')
