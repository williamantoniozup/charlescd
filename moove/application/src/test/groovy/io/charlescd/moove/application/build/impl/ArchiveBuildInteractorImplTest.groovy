...
class ArchiveBuildInteractorImplTest extends Specification {

    private ArchiveBuildInteractorImpl archiveBuildInteractor

    private BuildRepository buildRepository = Mock(BuildRepository)

    def setup() {
        this.archiveBuildInteractor = new ArchiveBuildInteractorImpl(new BuildService(buildRepository))
    }
    ...
    def 'when build does not exist should throw exception'() {
        ...
        when:
        archiveBuildInteractor.execute(buildId, workspaceId)
    ...
    def 'when build can not be archived because the deployment is status is deploying should throw exception'() {
        given:
        def workspaceId = '1a58c78a-6acb-11ea-bc55-0242ac130003'
        ...
        when:
        archiveBuildInteractor.execute(build.id, workspaceId)

        then:
        1 * buildRepository.find(build.id, workspaceId) >> Optional.of(build)

        def ex = thrown(BusinessException)
        ex.errorCode == MooveErrorCode.ARCHIVE_DEPLOYED_BUILD
    }

    def 'when build can not be archived because the deployment is status is deployed should throw exception'() {
        given:
        def workspaceId = '1a58c78a-6acb-11ea-bc55-0242ac130003'
        def author = getDummyUser()
    ...
    def 'should archive build successfully'() {
        given:
        def workspaceId = '1a58c78a-6acb-11ea-bc55-0242ac130003'
        def author = getDummyUser()
        ...
        then:
        1 * buildRepository.find(build.id, workspaceId) >> Optional.of(build)
        1 * buildRepository.updateStatus(build.id, BuildStatusEnum.ARCHIVED) >> _
    ...
    private static Build getDummyBuild(String workspaceId, User author, BuildStatusEnum buildStatusEnum, DeploymentStatusEnum deploymentStatusEnum) {
        ...
        componentSnapshotList.add(new ComponentSnapshot('70189ffc-b517-4719-8e20-278a7e5f9b33', '20209ffc-b517-4719-8e20-278a7e5f9b00',
                ...
                workspaceId, '3e1f3969-c6ec-4a44-96a0-101d45b668e7', 'hostValue', 'gatewayName','namespace'))
